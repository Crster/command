...
public sealed class ScreenCaptureHotkeyService : IDisposable
{
    ...
    public void Start()
    {
        if (_started)
        {
            Console.WriteLine("[ScreenCapture] Service already started");
            return;
        }

        Console.WriteLine("[ScreenCapture] Starting Screen Capture Hotkey Service");
        _started = true;
        _hookManager.Start();
        Console.WriteLine("[ScreenCapture] Hook manager started");
    }
    ...
    private void HookOnKeyPressed(object? sender, KeyboardHookEventArgs e)
    {
        lock (_sync)
        {
            ...
            {
                Console.WriteLine($"[ScreenCapture] Capture mode: Key pressed - KeyCode: {e.Data.KeyCode}");

                // Check for ESC key to restore last shortcut
                if (e.Data.KeyCode == KeyCode.VcEscape)
                {
                    Console.WriteLine("[ScreenCapture] ESC pressed - restoring last shortcut");
                ...
                {
                    Console.WriteLine("[ScreenCapture] Backspace pressed - restoring default shortcut");
                ...
                // In capture mode, format and return all key presses
                var keyName = GetKeyNameFromKeyCode(e.Data.KeyCode);
                var modifiers = GetModifiersFromMask(e.RawEvent.Mask);

                Console.WriteLine($"[ScreenCapture] KeyName: {keyName}, Modifiers: {string.Join(", ", modifiers)}");

                if (!string.IsNullOrWhiteSpace(keyName))
                {
                    var shortcutStr = FormatShortcut(modifiers, keyName);
                    Console.WriteLine($"[ScreenCapture] Formatted shortcut: {shortcutStr}");
        ...
        if (!TryParseShortcut(_storageService.GetScreenCaptureShortcut(), out var shortcut))
        {
            return;
        }
    ...
    public void Dispose()
    {
        ...
        {
            Console.WriteLine($"Error disposing ScreenCaptureHotkeyService: {ex.Message}");
        }
    }
